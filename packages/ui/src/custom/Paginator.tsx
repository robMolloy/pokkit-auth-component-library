...
export const Paginator = (
  p: {
    ...
    | { numberOfItems: number; itemsPerPage: number }
  )
) => {
  ...
  const numberOfPages =
    "numberOfPages" in p
      ? p.numberOfPages
      : Math.ceil(p.numberOfItems / p.itemsPerPage);
    ...
    <Pagination>
      <PaginationContent>
        ...
        {!visiblePageNumbers.includes(0) && pageNumber > 0 && (
          <PaginationItem>
            <PaginationLink onClick={() => setPageNumber(0)}>
              {1}
            </PaginationLink>
          </PaginationItem>
        )}
        ...
        <PaginationItem>
          <PaginationNext
            onClick={() =>
              setPageNumber((x) => (x === lastPageNumber ? x : x + 1))
            }
          />
        </PaginationItem>
      </PaginationContent>
